...
# ---- durable app settings (trailer profiles + options), shared across devices ----
...
def get_settings():
    try:
        ...
        print(f"[settings] read failed: {e}", flush=True)
    return jsonify({})
...
def post_settings():
    try:
        ...
        print(f"[settings] write failed: {e}", flush=True)
        return jsonify({"ok": False, "error": str(e)}), 500

@app.get("/")
def index():
    # serve the full-featured app if present, else fall back to index.html
    for name in ("cabinet-load-optimizer.html", "index.html"):
        if (Path(app.static_folder) / name).exists():
