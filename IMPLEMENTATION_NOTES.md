...
| Free-standing upright stability | Added a conservative V2 geometry guard for upright pieces standing directly on the trailer floor. | A pose is rejected when `height / min(width, depth)` exceeds `maxFreeStandingSlenderness` (currently 12); this is separate from tip-up clearance and does not infer bracing. |
